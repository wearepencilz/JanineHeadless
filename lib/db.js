...
export async function getTaxonomyValues(category) {
  ...
  if (!settings || !settings.taxonomies || !settings.taxonomies[category]) {
    return []
  }
  return settings.taxonomies[category]
}
...
export async function addTaxonomyValue(category, value) {
  ...
  if (!settings.taxonomies[category]) {
    settings.taxonomies[category] = []
  }
  
  // Generate ID and sortOrder
  const newValue = {
    id: value.id || value.value,
    label: value.label,
    value: value.value,
    sortOrder: value.sortOrder || settings.taxonomies[category].length + 1,
    archived: value.archived || false
  }
  
  settings.taxonomies[category].push(newValue)
...
export async function updateTaxonomyValue(category, id, updates) {
  ...
  const index = settings.taxonomies[category].findIndex(item => item.id === id)
  if (index === -1) {
    throw new Error(`Taxonomy value ${id} not found in category ${category}`)
  }
  
  settings.taxonomies[category][index] = {
    ...settings.taxonomies[category][index],
    ...updates
  }
  
  await saveSettings(settings)
  return settings.taxonomies[category][index]
}
...
export async function deleteTaxonomyValue(category, id) {
  ...
  settings.taxonomies[category].splice(index, 1)
  await saveSettings(settings)
  return true
}
